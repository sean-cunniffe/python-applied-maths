...
def writeToFile(data, name):
    np.savetxt(name + ".csv", data, delimiter=",", fmt='%f')


def loadFromFile():
    print('Loading data')
    titles = ['Approximation', 'Horizontal Detail', 'Vertical D'
                                                    'etail', 'Diagonal Detail']
    for i in range(4):
        file = titles[i]
        temp = np.loadtxt(open(file + ".csv", "rb"), delimiter=",")
...
def main():
    ...
    LL, (LH, HL, HH) = Haar_dwt2D(imgArray)
    writeToFile(LL, 'Approximation')
    writeToFile(LH, 'Horizontal Detail')
    writeToFile(HL, 'Vertical Detail')
    writeToFile(HH, 'Diagonal Detail')
    loadFromFile()
    print('finished')
    # reconstruct image from files (EXAM HINT)
    ######################  EXAM HINT   ######################################
